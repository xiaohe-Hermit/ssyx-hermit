...
public class AttrGroupController {
    ...
    @ApiOperation("获取属性分组分页")
    @GetMapping("{page}/{limit}")
    public Result getPageList(@PathVariable Long page, @PathVariable Long limit, AttrGroupQueryVo attrGroupQueryVo) {
        Page<AttrGroup> pageParam = new Page<>(page,limit);
        IPage<AttrGroup> pageModel1 = attrGroupService.selectAttrGroupPage(pageParam,attrGroupQueryVo);
        return Result.ok(pageModel1);
    }

    @ApiOperation("查询所有属性分组列表")
    @GetMapping("findAllList")
    public Result findAllList(){
        List<AttrGroup> list = attrGroupService.selectAllAttrGroupList();
